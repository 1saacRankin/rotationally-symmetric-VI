# Variational inference exactly recovers the mean when the target is rotationally symmetric
...
ggplot(data = plot.data, aes(x = z1, y = z2, color = type)) +
  ...
  geom_point(data = plot.data2, aes(x = mean_z1, y = mean_z2, color = type, stroke = 1),
             size = 8, shape = 10) +
  xlim(-6, 10) +
  ylim(-7, 9) +
...
# Plot points, contours, means
ggplot(data = plot.data, aes(x = z1, y = z2, color = type)) +
  geom_density_2d(alpha = 1, linewidth = 0.5) +
